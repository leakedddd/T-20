...
class CartFragment : Fragment() {
            ...
            onQuantityChanged = { productId, newQuantity ->
                viewLifecycleOwner.lifecycleScope.launch {
                    withContext(Dispatchers.IO) {
                        ...
                        if (userEmail != null) {
                            try {
                                firebaseRepo.saveCartItem(userEmail, CartItem(productId, newQuantity))
                            } catch (e: Exception) { }
                        }
                    }
                    loadCart()
            ...
            onRemove = { productId ->
                viewLifecycleOwner.lifecycleScope.launch {
                    withContext(Dispatchers.IO) {
                        ...
                        val userEmail = prefs.getString("user_email", null)
                        if (userEmail != null) {
                            try { firebaseRepo.removeCartItem(userEmail, productId) } catch (e: Exception) { }
                        }
                    }
                    loadCart()
                }
    ...
    private fun loadCart() {
        viewLifecycleOwner.lifecycleScope.launch {
            val (cartItems, totalItems, totalPrice) = withContext(Dispatchers.IO) {
                Triple(
